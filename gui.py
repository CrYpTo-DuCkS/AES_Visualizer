...
def run_gui(grid, text, col, key, sub, col2, xor, board, xor_key, sub_bytes, mix_cols):
    pygame.init()
    ...
    while True:
        ...
        for i in range(4):
            if mix_cols[i].get_show():
                screen.blit(mix_cols[i].col, get_object_location(mix_cols[i].get_location(), camera.offset))
